      ...
      <form onSubmit={ode} className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start">
        ...
        <aside className="rounded-2xl border border-gold/20 bg-night p-5 lg:sticky lg:top-32">
          ...
          <button
            type="submit"
            disabled={yuk}
            className={`mt-4 w-full rounded-full py-3 font-medium transition-colors disabled:opacity-60 ${
              mod === "whatsapp"
                ? "flex items-center justify-center gap-2 bg-[#25D366] text-white hover:bg-[#1eb858]"
                : "bg-gold text-night-deep hover:bg-gold-bright"
            }`}
          >
            {yuk ? (
              "İşleniyor…"
